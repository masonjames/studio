...
try {
    $skipSigning = $env:STUDIO_SKIP_SIGNING -eq "true"
    if ($skipSigning) {
        Write-Host "--- :information_source: Skipping Windows code signing setup because STUDIO_SKIP_SIGNING=true"
    } else {
    ...
    Write-Host "--- :electron: Writing release config"
    ...
    git checkout -- .\apps\studio\src\release-config.ts *> $null
}
